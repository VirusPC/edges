...
- 现有阿里云 ECS（与 teach 同机）可以用 nginx 把 `/health` 和 `/artifacts/` 反代到本机 8787，对外 BASE_URL 是 `http://182.92.131.89`（不另开公网端口，也不动 `/teaching/`）。一次性：人跑 `setup-nginx-artifacts.sh`、把 token 放进盒上 `artifacts-preview.env`；之后本机 `edges artifacts init --base-url http://182.92.131.89` 再 `edges artifacts publish`。合并到 main 后，`deploy-teach.yml` 在整仓 pull 之后会跑 `deploy/bootstrap.sh` 重启该服务。
