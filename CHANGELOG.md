...
- `edges tasks project review-page` 和 `/tasks/` 上，三栏章节标题是 Projects、Tasks、Details。状态列名仍是 backlog 一类原值，点开的任务名留在 Details 下面。章节标题比列表和卡片更大、更粗，放在浅底色条上，下面有一条分区线；窄屏和宽屏同一套。
