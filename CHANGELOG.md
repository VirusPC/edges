...
- CONTEXT 增加 Task Project（edges）、`edges-task-project`；收紧 Task / `edges-tasks-status` / `edges-task-priority` / edges tasks（CLI）：分组在 `knowledge/tasks/<project-slug>/`，未分组 `_default`；目录与 frontmatter 双写；`status` 只在同一 project 内移动。后续跨 project 用 `update --project`（或等价入口）。
- `docs/adr/0008-edges-task-project-grouping.md`：directory-first 的 Multica-like Project + frontmatter 双写；迁移后 tasks 根下不再直接放 status 夹。本轮只定文档，不迁看板、不改 CLI；实现轮必须先让 CLI 跟上。
