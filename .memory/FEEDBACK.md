...
- [classifyTasks 是 NCC，不是发现簇](feedbacks/feedback_classify_tasks_ncc_not_clustering.md) — 写或改 project-tasks-classify / classifyTasks 时：方法是 Embedding-based NCC，质心由用户预先设定；不要写成聚类发现或迭代更新质心。缘起 https://github.com/VirusPC/edges/pull/78。
- [reference 的 description 必须带关键链接](feedbacks/feedback_description_must_include_urls.md) — 写或更新 .memory/references/* 时：description 与 REFERENCE.md 索引行必须带关键 URL，不能只写在正文 Links。缘起 https://github.com/VirusPC/edges/pull/45。
