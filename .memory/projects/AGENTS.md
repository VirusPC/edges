...
- [conversation-to-tasks：背景+目标必填，人审后落库](project_conversation_to_tasks_body_review_persist.md) — 改 conversation-to-tasks 或从对话开卡时：背景→目标必填；完成标准与动作可选（完成标准可留到 grill-with-docs）；必填不足先问；成文后交人审再 CLI 落库；不限制是否新建分支。
