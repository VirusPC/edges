...
- [ECS 上 edges 用 Actions SSH 整仓 pull](project_teach_site_rsync_push.md) — 改 teaching、/tasks/ 或 ECS 部署时：Actions SSH 整仓 pull，不要 rsync；environment 保持 production，url 为 https://edges.viruspc.tech/teaching/，summary 同时列 /tasks/；不要用 teach.* 或裸 IP 当对外入口。不要新开 workflow（ADR 0021）。
