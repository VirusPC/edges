...
- [Task Project 审阅页是 render-only CLI](project_task_project_review_page_render_only_cli.md) — 改审阅壳或 edges tasks project review-page 时打开：仍只渲染、无 --mode。ADR 0022：产物不入库（build:review-app / prepack / CI / ECS 生成），内联单 HTML 写入 #edges-review-payload，hash-router；源码 extensions/clis/review-app/，产物 assets/review-page/。决策见 docs/adr/0012 与 docs/adr/0022。
