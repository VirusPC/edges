...
- [个人 Artifacts 预览服务：上传→URL→TTL](project_artifacts_preview_service.md) — 改 Artifacts 预览服务、edges artifacts、或审阅页如何给人打开时打开：短生命周期静态托管（上传→URL→TTL）；review-page 仍只渲染；结果回传另卡。决策见 docs/adr/0013-artifacts-preview-service.md。
...
- [Task Project 审阅页是 render-only CLI](project_task_project_review_page_render_only_cli.md) — 改 classifyTasks / proposeTypes 人闸或 edges tasks project review-page 时打开：CLI 只渲通用 groups+items HTML（已落地）；托管/发布见 ADR 0013，不要把 publish 并进 review-page。决策见 docs/adr/0012-task-project-review-page-is-render-only-cli.md。
