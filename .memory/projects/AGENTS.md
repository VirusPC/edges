...
- [Task Project 审阅页是 render-only CLI](project_task_project_review_page_render_only_cli.md) — 改审阅壳或 edges tasks project review-page 时打开：仍只渲染、无 --mode。桌面三栏见 ADR 0022。窄屏同一页纵向分段；章节头是顶栏和背景之间的过渡色面，状态行贴着页面背景且比章节小一档，强色在顶栏的 Layers2 图标块和色带。窄屏项目 Select 用 popper 锚在触发器上。筛选抽屉右上角是 ×。不要视口面板。源码在 apps/tasks-review-app/。
