...
- [Task Project 审阅页是 render-only CLI](project_task_project_review_page_render_only_cli.md) — 改审阅壳或 edges tasks project review-page 时打开：仍只渲染、无 --mode。桌面三栏见 ADR 0022。窄屏看板纵向分段；点卡片后详情是盖住看板的视口面板，回到看板不取消选中，顶栏不在正文滚动层里。源码在 apps/tasks-review-app/。
