...
- [Task Project 审阅页是 render-only CLI](project_task_project_review_page_render_only_cli.md) — 改审阅壳、classifyTasks / proposeTypes 人闸或 edges tasks project review-page 时打开：仍只渲染、无 --mode、Copy JSON 落地；ADR 0022 把同一壳定为三栏（左栏只改 project、中栏状态只读、右栏读页内 Task Doc）。决策见 docs/adr/0012 与 docs/adr/0022。
