...
description: 改审阅壳或 edges tasks project review-page 时打开：仍只渲染、无 --mode。桌面三栏与预构建见 ADR 0022。窄于 Tailwind md（<768px）为纵向长滚动，双端「移到项目…」只改页内 JSON，见 ADR 0023。源码在 apps/tasks-review-app/。
metadata:
  ...
  edges-updated-at: "2026-09-24T14:40:20+00:00"
---
...
2026-09-24 ADR 0023 再修订同一份壳的窄屏，不重开 render-only。窄于 Tailwind `md`（`<768px`）是纵向长滚动：项目筛选区，然后状态板，点卡片后详情接在状态板下方。`≥768px` 保持 #126 三栏（可拖分隔线、左栏拖放），只加每张卡的「移到项目…」。菜单与左栏拖放都只改页内 JSON，Copy JSON 仍是落地。窄屏触摸拖放可选。用户所述，grill 确认于 2026-09-24；本轮只落文档，React 尚未按这条改。

**Why:**
2026-09-17 grill 确认：ad-hoc HTML 与聊天 Markdown 表不够当交互闸门；Grok Bot HTML 预览里拖拽不可靠。CLI 若计算归属会重开 ADR 0010 已否决的 `classify` 动词。`--mode` 会把页绑死在 classify vs propose，而两组工作流只要同一壳。能力面仍是 CLI + Skill + MCP。2026-09-19 ADR 0013 只改打开方式，不改 render-only。2026-09-21 ADR 0021 再加一条持久入口，仍不改 render-only。2026-09-23 若为看板另开顶层 schema，或让浏览器读仓内 `.md`，会把 list 契约和页内文档拆成两套；状态写回与 project 拖拽也不是一件事。安装后现编会把 Vite 工具链变成用户依赖，也让 `review-page` 变成一次构建。只学 prebuild、把 React 源码和 CLI 命令放在同一目录，运行时就会 import 到源码树，和 Playwright reporter 只读 `lib/vite/htmlReport` 的包内布局不一致。2026-09-24 窄屏若另做一壳、或做成盖住看板的抽屉 / 底栏 / 滑页，会把同一份壳拆开；390×844 上硬挤三栏已经不可用。菜单只改页内 JSON，是为了窄屏也能改 project，同时不碰 ADR 0012 的 Copy JSON 落地。

**How to apply:**
- How-to: plan at docs/superpowers/plans/2026-09-17-task-project-review-page.md；壳的三栏与 `doc` 以 ADR 0022 为准，窄屏布局与「移到项目…」以 ADR 0023 为准。不要按旧的两栏拖拽改状态。
- 改 glossary、审阅交互或 classifyTasks / proposeTypes 人闸时按 ADR 0012 / 0022 / 0023 与 CONTEXT 术语审阅壳 / Task Doc / doc（看板条目）/ Task stem。
- 窄于 `md`：纵向长滚动（项目筛选区 → 状态板 → 详情接在下方，同一页滚动，打开详情时滚到详情，可以有「回到看板」锚点）。状态列在状态板内部横向滚，整页不横向溢出。每张卡的「移到项目…」是必须的；触摸拖放可以跳过。不要做成盖住看板的抽屉、底栏分页或左右滑页。
- `≥768px`：保持 #126 的三栏、可拖分隔线、左栏拖放。观感只允许多出卡片菜单。桌面右栏（外观收口称 Markdown 抽屉）留在三栏里。
...
- 对照 ADR `docs/adr/0012-task-project-review-page-is-render-only-cli.md`、`docs/adr/0022-review-shell-three-column-task-doc.md`、`docs/adr/0023-review-shell-narrow-vertical-layout.md`、`docs/adr/0013-artifacts-preview-service.md` 与 `docs/adr/0021-persistent-tasks-board-site.md`。
