...
description: 改审阅壳或 edges tasks project review-page 时打开：仍只渲染、无 --mode。桌面三栏见 ADR 0022。窄屏看板纵向分段；点卡片后详情是盖住看板的视口面板，回到看板不取消选中，顶栏不在正文滚动层里。源码在 apps/tasks-review-app/。
metadata:
  ...
  edges-updated-at: "2026-09-25T00:20:07+00:00"
---
...
2026-09-24 ADR 0023 把窄屏看板改成纵向长滚动，双端「移到项目…」只改页内 JSON。同一天设计验收否掉「详情接在看板下面、用文档流吸顶条」：窄屏点开卡片后，详情是盖住看板的视口面板，铺在 Edges/筛选条下面（`position: fixed`，占满剩余视口）。面板头固定「回到看板」和当前标题（过长省略），并且必须在正文滚动层外面。面板自己 `overflow: hidden`，挂到 `document.body`；只有正文那一层 `overflow-y: auto`。`html` / `body` / `#root` 用 `overflow: hidden`。不要只写 `overflow-x: hidden`，那会把 `overflow-y` 算成 `auto`，顶栏的祖先一滚，回到看板就跟着走。回到看板关掉面板，不改 hash、不取消选中。看板本身仍是纵向分段。这一轮按用户要求不改 ADR 正文。
...
- 窄于 `md`：项目筛选是一个下拉；状态板按中文状态纵向分段，只显示有卡片的状态且全部展开，整页不横向溢出。点卡片进入详情：视口级面板盖住看板。结构锁死为：面板 `fixed` 铺满 Edges 下面的视口，`flex flex-col` 且 `overflow: hidden`；头 `shrink-0`（回到看板 + 标题）不在 `overflow-y-auto` 里；只有正文层 `flex-1 min-h-0 overflow-y-auto`。不要把回到看板和正文放进同一个滚动层再靠 `sticky`。回到看板只关面板。不要做成底栏分页或左右滑页。每张卡的「移到项目…」是必须的；触摸拖放可以跳过。
