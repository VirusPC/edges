...
description: 改 /tasks/ 持久入口、list --group-by、或看板站 vs Artifacts 时打开：不新开 status station；CI 扩展 deploy-teach.yml 并在 pull 后生成 _site；分组 schema 松耦合（edges.tasks.grouped/v1）；review-page 仍只渲染；nginx 一次性 setup-nginx-tasks.sh。决策见 docs/adr/0021-persistent-tasks-board-site.md。
metadata:
...
`/tasks/` 是与 teaching 同机的持久看板入口，始终反映 main：`edges tasks list --group-by project` 产出松耦合 `edges.tasks.grouped/v1`，薄映射后喂现有 `review-page`；CI 扩展 `deploy-teach.yml`，不新开 status station。已落地（2026-09-21 实现轮）。
...
- 生成：盒上 `pnpm --filter edges-cli exec -- tsx scripts/generate-tasks-site.ts --out "$PWD/knowledge/tasks/_site/index.html"`（默认相对路径 `knowledge/tasks/_site/index.html`，gitignored）。`deploy-teach.yml` 在 `reset --hard origin/main` 之后始终生成；失败则整次 SSH 失败。不要把 generate 绑在 artifacts env 上。
