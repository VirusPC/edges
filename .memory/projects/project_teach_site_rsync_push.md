...
**Why:** 用户确认国内 ECS 现已能经 HTTPS 访问 GitHub（git fetch 可用），并要求整仓 pull，而不是只同步 knowledge/teaching。仓库 secrets `TEACH_DEPLOY_SSH_KEY` / `TEACH_DEPLOY_HOST` / `TEACH_DEPLOY_USER` 仍复用。静态文件改完磁盘即可，不必 reload nginx。
