...
description: 改 Artifacts 预览服务、edges artifacts、或审阅页如何给人打开时打开：稳定短生命周期托管 + 真浏览器可开 URL；服务在 extensions/services/artifacts-preview；CLI 是 edges artifacts init|publish|rm；from 按 kind 判别（task 用 project+stem，其他用 name）；review-page 仍只渲染；结果回传另卡。决策见 docs/adr/0013-artifacts-preview-service.md。
metadata:
...
v1 Artifacts 预览服务已落地：`extensions/services/artifacts-preview/` 常驻 HTTP（上传→URL→TTL 删除），`edges artifacts init|publish|rm` 是薄命令面；`review-page` 仍只渲染；Skill 渲染后 `publish` 再给人可达 URL。手机审阅必须用 ECS / 可达地址。本轮没有 artifacts MCP，也不做审阅结果回传。新 publish 必带 `from`，按 `kind` 判别：`task` 用 `{ kind, project, stem }`（不要 name）；其他 kind 用 `{ kind, name }`（不要 project/stem）。`meta.json` 只有 `id` / `entry` / `expiresAt` / `from`。CLI 默认 `{ kind: "cli", name: "edges-cli" }`；`--from-kind task` 必须带 `--task-project` / `--task-stem`。

**Why:**
2026-09-19 grill（peng cheng）定 ADR 0013；实现按 `docs/superpowers/plans/2026-09-19-artifacts-preview-service.md`。2026-09-19 先要求结构化 `from`。2026-09-20 一度加过顶层 `task`，同日纠正为 `from.kind === "task"`。
...
- 来源是看板 Task 时用 `--from-kind task --task-project <slug> --task-stem <stem>`，不要同时 `--from-name`，也不要写顶层 `task`。
- 服务端按 kind 校验；缺/空/`..`/路径分隔符是 400。
