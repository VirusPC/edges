...
个人 Artifacts 预览服务是短生命周期静态托管：上传 → URL → TTL 删除；`edges artifacts` 薄命令面（init/token，publish/rm）；`review-page` 仍只渲染；Skill 渲染后发布再给人可达 URL。手机审阅必须用 ECS / 可达 URL。写要共享 token，读靠 UUID 路径 + TTL（默认 24h）。本轮只定 CONTEXT / ADR 0013，不实现。

**Why:**
2026-09-19 grill 确认（peng cheng）：聊天 HTML 预览不可靠（手机常打不开，桌面交互坏）。不是长期建站，不是本地 HTML 视图，也不是审阅结果回传 Agent 客户端（已拆独立 backlog）。
