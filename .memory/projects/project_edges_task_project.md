...
description: 看板 Task Project 为 directory-first + frontmatter 双写；未分组用 _default。决策见 docs/adr/0009-edges-task-project-grouping.md（修订 ADR 0002 路径）。本轮只定文档，不迁看板、不改 CLI；后续 status 只在同 project 内移动。
metadata:
  ...
  edges-updated-at: "2026-09-16T03:21:35+00:00"
---
...
**Why:**
2026-09-16 grill 确认要对齐 Multica Project，但不能把状态夹当 project，也不能只改标签不改路径。ADR 0009 修订 ADR 0002 的路径：状态夹仍按 `edges-tasks-status`，但嵌在 project-slug 下。一次性迁移现有 `tasks/<status>/*` → `tasks/_default/<status>/*` 后，tasks 根下不再直接放 status 夹。本轮不做 parent/sub-issue/stage，也不改写 Skill/MCP；实现轮必须先让 `edges tasks` CLI 跟上新路径。能力面仍是 CLI + Skill + MCP。决策见 `docs/adr/0009-edges-task-project-grouping.md`。
...
- 对照 ADR `docs/adr/0009-edges-task-project-grouping.md`；叠 ADR 0002 / 0005 / 0007。
