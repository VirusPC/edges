...
- [Task 看板按 Task Project 目录优先分组](projects/project_edges_task_project.md) — 看板 Task Project 为 directory-first + frontmatter 双写；未分组用 _default。决策见 docs/adr/0008-edges-task-project-grouping.md（修订 ADR 0002 路径）。本轮只定文档，不迁看板、不改 CLI；后续 status 只在同 project 内移动。
