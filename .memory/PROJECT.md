...
- [可扩展 Memory Type：LAYOUT 登记，不另开注册表](projects/project_extensible_memory_types.md) — 可扩展 Memory Type：扩展面只在 LAYOUT，用 project-memory-add-type 登记；官方种子仍是六类。实现计划见 docs/superpowers/plans/2026-09-13-extensible-project-memory-types.md。决策见 docs/adr/0006-extensible-project-memory-types.md。
...
- [ECS 上 edges 用 Actions SSH 整仓 pull](projects/project_teach_site_rsync_push.md) — 改 teach 站点或 ECS 上的 edges 部署时：用 GitHub Actions SSH 触发整仓 git fetch/reset，不要再 rsync 推送。
