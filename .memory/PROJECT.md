...
- [ECS 上 edges 用 Actions SSH 整仓 pull](projects/project_teach_site_rsync_push.md) — 改 teach 站点或 ECS 上的 edges 部署时：用 GitHub Actions SSH 触发整仓 git fetch/reset，不要再 rsync 推送；deploy job 保持 environment: ecs，根 README 保留工作流徽章。
