...
- [classifyTasks 与 Task Project 元数据实现计划](projects/project_classify_tasks_and_project_metadata.md) — 实现或改 edges tasks project / classify-tasks Skill 时打开：计划在 docs/superpowers/plans/2026-09-17-classify-tasks.md；能力面 CLI + Skill + MCP；无 embedding。Skill 正文必须保留禁止句 edges tasks classify，探针不要因此失败。
