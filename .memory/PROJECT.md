...
- [对话整理采用复盘四栏](projects/project_conversation_notes_fupan_four_columns.md) — 改 conversation-to-notes 或对话 Note 结构时：用复盘四栏；不要复活已关闭的 FIA 中文换皮（事实/洞察/行动）；不要批量改写旧笔记；不要把升 Edge 写进该 skill。
