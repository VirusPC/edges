...
- [task 是 from.type，不是顶层字段](feedback_artifact_from_task_is_kind.md) — 改 artifact meta / edges artifacts publish 的 from 时打开：task 是 from.type 的一种来源，不要再写顶层 task 或 from.kind；type=task 用 project+stem，其他 type 用 name；CLI 是 --from-type。
