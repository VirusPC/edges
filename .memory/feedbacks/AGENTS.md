...
- [task 是 from.kind，不是顶层字段](feedback_artifact_from_task_is_kind.md) — 改 artifact meta / edges artifacts publish 的 from 时打开：task 是 from.kind 的一种来源，不要再写顶层 task {project,stem}；kind=task 用 project+stem，其他 kind 用 name。
