...
- [learn-repo](../.agents/skills/learn-repo/SKILL.md) — 当用户想长期学习、精读某个外部代码仓库，并把它关联进 knowledge/teach 教学工作区时使用。以 git submodule 把仓库挂到对应主题的 repos/ 下——主仓库只记一个提交指针，不涨克隆体积——并在 RESOURCES.md 登记来源与 commit。触发短语如「我想学一下 XX 仓库」「把 XX 仓库挂进来学」。只是临时看看、总结一下某个仓库时不触发；往 knowledge/teach 之外的路径挂载也不归本技能管。
