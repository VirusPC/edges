---
name: looki-memory
description: "Access the user's Looki L1 digital memory (moments, journals, lifelog, and For You highlights, including what happened on a given date) so answers can use that real-world context. Do not use this for Edges knowledge notes, or for the Looki SEA shopping MCP."
---
...
- Upstream skill: https://web.looki.tech/agent/looki-memory/SKILL.md (also at looki.ai)
- Developer platform base URLs observed: `https://open.looki.tech/api/v1` (CN web config) and `https://open.looki.ai/api/v1` (intl docs). Always use the `base_url` from credentials / the user; do not hardcode.
- API references UI: https://web.looki.tech/api-references
- Do not commit API keys. Credentials stay only in `~/.config/looki/credentials.json`.
