...
```json
{ "id": "<uuid>", "entry": "index.html", "expiresAt": "2026-09-20T14:00:00.000Z" }
```
...
```json
{"status":"success","command":"artifacts.publish","id":"<uuid>","url":"http://127.0.0.1:8787/artifacts/<uuid>/","expiresAt":"…"}
```
...
```bash
pnpm --filter edges-cli exec tsx src/index.ts artifacts publish <absolute-html-path>
```
