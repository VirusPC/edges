...
- **按小型常开服务器运维。** 2026-09-20 第三轮：minigtr 多数时候当小型服务器开着跑 Langfuse。双系统仍在，但 Windows 不是日常路径；不要按「经常切回 Windows / 当桌面机」来设计可用性。
...
- 按日常 Windows 桌面机规划（频繁重启或切系统）：否决第三轮；双系统还在，但不是正常路径。
- 本轮部署或把密钥/compose 写入本仓：否决。

## Consequences

- 实例可用性按 Linux 侧多数时候在线来假设。切到 Windows 会停掉这台机上的 Langfuse，但这不是日常路径，不单独为此做双系统热切换。
- 访问策略不在本 ADR 展开：v1 仅 Tailscale；公网 HTTPS 是后续（ADR 0017）。在那之前，没有 Tailscale 就只剩 minigtr 的 localhost / 局域网——第三轮确认接受这段缺口。
- 密钥与备份见 ADR 0018 / 0019，不在本 ADR 规定目录或卷名。
- v1 怎么验收（先 UI，再 1–2 个客户端）见 ADR 0020。
