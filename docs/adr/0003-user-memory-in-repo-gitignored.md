...
生命周期跟随工作树（`rm -rf` 克隆可以删掉它）。逃生靠备份 skill 与恢复/回注 skill：备份目标目录由用户指定，默认归档落在仓库根。gitignore 覆盖 `.memory/users/`、根索引 `.memory/USER.md`（与其他类型索引同位置），以及仓库根默认备份名 `user-memory-backup-*.tar.gz` / `user-memory-backup-*.zip`。
