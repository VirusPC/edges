...
Default TTL is 24h (`ttlSeconds` on POST, or `edges artifacts publish --ttl 2h`). Server sweeps expired artifacts about every 60s and also on GET. Writes stay under a disk temp dir; relative paths cannot traverse or follow symlinks out.
