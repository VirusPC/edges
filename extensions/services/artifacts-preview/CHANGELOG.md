...
- ECS runbook in `deploy/`: systemd user unit, `artifacts.env.example`, nginx snippet (`/health`, `POST /artifacts`, and `/artifacts/…` on :80, `/teaching/` untouched), `setup-nginx-artifacts.sh` (human sudo once), `bootstrap.sh` (install/build/restart). `deploy-teach.yml` runs bootstrap after the full-repo pull only when the server env file exists.
