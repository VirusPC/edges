...
- `setup-nginx` still injects only into server blocks that already contain `/teaching/`. If Aliyun `teach.conf` still serves legacy `/teach/`, run `deploy/migrate-teach-nginx-prefix.py` (then `nginx -t` / reload) before `edges artifacts server setup-nginx`. Do not dual-match `/teach/` in the injector.
