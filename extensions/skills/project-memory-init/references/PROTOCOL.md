...
flowchart TD
    ...
    subgraph localMem [本层记忆]
        EntryA["MEMORY_TYPE_A/AGENTS.md"]
        EntryB["MEMORY_TYPE_B/AGENTS.md"]
        MemA1["memory_1.md"]
        MemA2["memory_2.md"]
        MemB1["memory_3.md"]
        EntryA --> MemA1
...
## `MEMORY_TYPE_*/AGENTS.md` — 本层不同类型记忆入口

- `MEMORY_TYPE_*/AGENTS.md` 是**本层不同类型的记忆入口**：位于该类型的内容目录下，文件名是 `AGENTS.md`，仅含该类型的索引地址和描述，不含具体记忆内容。
- 它与层入口同名，契约不同：类型入口不是一层记忆的入口，不写本层硬约束，也不列下层记忆。
- 每个 `type` 一份，只列该类型的记忆文件——一条记忆归在哪份入口下，就是它的 `type`。所以层入口 `AGENTS.md` 到本层任一条记忆**固定两跳**。
...
图上和小节标题里的名字、数量都是示意。标记名、有哪些 `type`、类型内容目录叫什么、上面没列的字段，都属于实现，会随版本变——**按实际读到的内容走**，看不到这个形状就当项目还没建记忆。类型入口的文件名是 `AGENTS.md`、存在类型内容目录下，这是形状，不是某个具体 type 的名字。

破协议就是破坏图上的形状：

- 记忆平铺在某一层，或 `AGENTS.md` 不列直接下层、断掉递归。
- 层目录或类型内容目录的入口不再是 `AGENTS.md`。
- 抽掉记忆入口这一层，记忆文件直接挂在层入口 `AGENTS.md` 下。
- 把类型入口写成层入口，或把层入口写成只含一类条目。
