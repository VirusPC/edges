...
- `AGENTS.md` 是**本层记忆入口**，仅含索引文件地址和描述，不含具体记忆内容。
- 分两部分内容：本层记忆和下层记忆。不平铺整棵树——顺着下层边一层层跳，就能走遍。
- 两层入口都一样：**按描述挑下一跳**，不要把一层整个拉进来。
...
- 它是共享文件：工具只拥有自己用标记划出的区域，区块之外的正文一律保留，重复执行只刷新区块内。
