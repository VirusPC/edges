...
[`PROTOCOL.md`](PROTOCOL.md) 的实现。三个文件小节与协议一一对应，破折号后的解释语与协议一致。
...
├── .memory/
│   ├── FEEDBACK.md                 # 分类型入口：只列 feedback_*.md
│   ├── PROJECT.md                  # 分类型入口：只列 project_*.md
│   ├── REFERENCE.md                # 分类型入口：只列 reference_*.md
...
| ├ `<!-- project-memory-local:start -->` | 每个记忆目录 | 本层三份分类型入口的清单 | 模板里的字面量 |
...
## `FEEDBACK.md` / `PROJECT.md` / `REFERENCE.md` — 本层不同类型记忆入口

协议要求按 `type` 分入口，本实现取三类，都放在本层 `.memory/` 下。各有一个 `<!-- project-memory-entries:start -->` 区块，内容从记忆文件的 frontmatter 全量重算。

**条目**指区块里的一行，与记忆文件一一对应。入口是派生产物、不手写；行格式只存在于 [`templates/_entry_line.tmpl.md`](templates/_entry_line.tmpl.md)，下层索引与条目索引共用。

`type` 沿用 [Claude Code auto memory](https://code.claude.com/docs/en/memory)，取其中三类，官方第四类 `user`（角色、专长、个人偏好）不落盘。每类「记什么、不记什么」写在对应入口模板的引言里。

| type | 记忆入口 | 记忆文件 | 收什么 |
| --- | --- | --- | --- |
| `feedback` | `FEEDBACK.md` | `feedback_<slug>.md` | 用户的纠正、确认过的做法、禁止模式 |
| `project` | `PROJECT.md` | `project_<slug>.md` | 进行中的工作、时间点、代码里推不出的决策 |
| `reference` | `REFERENCE.md` | `reference_<slug>.md` | 项目外的信息去哪找 |

## `<type>_<slug>.md` — 详细记忆内容

`slug` 是小写 snake_case 且不带类型前缀，前缀由脚本按 `type` 加。
...
**模板名 = 产物名去掉后缀 + `.tmpl.md`**。下划线开头的是行片段，不对应产物；`type_slug.tmpl.md` 是唯一例外，产物名带尖括号，文件名改用角色词。

`AGENTS.tmpl.md` 把四对区块标记连嵌套关系一起写在里面。其中本层记忆区块的每一行就是一个类型声明，脚本从中推导 `type` 与索引文件名，所以新增一个类型只需补一行再放一份同名模板。索引文件名全大写，条目前缀取它的小写。
