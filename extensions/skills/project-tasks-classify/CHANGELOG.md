...
- 第 4 步在 `review-page` 渲出本地 HTML 之后：若人需要可达 URL，再 `edges artifacts publish --from-kind skill --from-name project-tasks-classify` 并把公开 `url` 给人（手机不能假定 localhost）。来源是看板 Task 时改用 `--from-kind task --task-project` / `--task-stem`。`review-page` 仍只渲染。不实现审阅结果回传 Agent 客户端。本轮没有 artifacts MCP。
