...
description: 把已有 AGENTS.md 按 project-memory-init 的形状重新组织：常驻指令留在区块外正文，记忆内容抽到 .memory，受管区块只留索引。用户要求整理、改造、迁移、重组已有 AGENTS.md 时使用；init/doctor 不改正文，不要用它们代替本 skill。
version: 1.0.0
---

# Project Memory Reshape

把一份**已经存在**的 `AGENTS.md` 收成 init 要求的形状：受管区块是索引，区块外是人写的常驻指令，记忆正文在 `.memory/` 条目里。
...
协议禁止把记忆内容塞进 `AGENTS.md`。区块外正文会无条件常驻，所以只留**不检索就会做错事**的指令。每一段只进一处：

| 去哪 | 收什么 |
| --- | --- |
| 留在 `AGENTS.md` 区块外 | 身份与职责、动手前必须看到的硬约束、指向唯一真理源的指针（例如「目录约定看 README」） |
...
6. 改 `AGENTS.md` 时**只动受管区块之外的正文**。受管区块交给 init / remember / doctor 刷新，不要手写索引行。别的工具的受管块（标记名不是 `project-memory`）原样保留。
...
- 不要为了「整齐」把常驻硬约束抽空——抽空后 agent 不再加载 remember，约束就从每次会话里消失。
