...
- [new_note 收成 extensions/clis/edges，MCP 保留](projects/project_clis_from_mcp.md) — 改 note ingest、new-note MCP 或 clis 时：本地 agent 走 extensions/clis 的 edges note（无 edges-note shim，根目录不默认 ingest）；git 仍在 bin/new-note，MCP 保留；鉴权 flag 留在 note 上（--token-file / --token-stdin），JSON stdout。不要把 CLI 放仓库根。
