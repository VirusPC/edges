...
- 新增一个远程 MCP server 能力（落地在 `extensions/mcp-servers/new-note`），用于接收外部 AI 传入的总结内容与元数据。
...
- Affected code: `extensions/mcp-servers/new-note` 下的 TypeScript MCP server 实现代码、命令执行与参数校验模块、可能的 `bin/new-note` 兼容性调整。
