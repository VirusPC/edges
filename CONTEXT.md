...
**对话三角色（笔记 / 记忆 / 任务）：**
同一段对话可以分出三类产物，不要揉成一张：`conversation-to-notes` 记「这次澄清了什么」（复盘四栏）；`project-memory-remember` 记「以后还该记住什么」（结论 → 为何 → 做法）；`conversation-to-tasks` 记「谁下一步做什么、怎样算完」（结论 → 事实背景 → 为何 → 做法；事实背景须含场景）。三个整理技能都只成文；写入笔记仓、记忆树或任务看板，各自是另一步。
_避免使用_：把任务写成记忆结论；把复盘笔记写成看板待办；在整理技能里直接调用命令行写盘；正文栏名中英混写
