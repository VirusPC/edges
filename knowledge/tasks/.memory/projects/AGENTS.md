...
- [tasks 工作流阶段：grill → plan → implement](project_assign_grill_with_docs_first.md) — 写或派发 tasks 时：阶段为 idea→落盘→出栈指派→grill→plan→implement→收口；grill、plan、implement 是不同阶段，grill 不能顶替 plan；实现阶段名用英文 implement；派发默认先 grill 再 plan 再 implement，用户当次跳过细聊除外。
