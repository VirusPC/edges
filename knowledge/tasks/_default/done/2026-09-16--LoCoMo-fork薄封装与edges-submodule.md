...
- **完成（2026-09-16）：** PR #72 钉 VirusPC/locomo submodule `cb5151e` + `evaluation/run_locomo_official.py` 调官方 `evaluate_qa.py` / `evaluation.py` F1。`evaluation/cases/locomo-smoke` 标 legacy。四臂仍 paused。**仍是 Evaluation Smoke，不是 Benchmark Proof / Project Memory 证明（ADR 0008）。**
