...
- `.github/workflows/deploy.yml` 仍设 `environment.name: production` 且 `url: http://182.92.131.89/teaching/`；同 job 已生成 tasks `_site`，但 GitHub 每个 environment 只展示一个 url。
...
**How to apply:**
- grill 单 url vs 双 environment vs README badge；改 deploy.yml。
