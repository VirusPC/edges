...
- 用户同意：同一套 edges-artifacts-preview 挂到现有 teach 同机阿里云 ECS（公网 http://182.92.131.89）：GitHub Action SSH 拉 main → Node 服务 → nginx 反代 `/artifacts` 与 `/health`；一次 nginx/sudo 可能需本机手工（同 teach 先例）。
