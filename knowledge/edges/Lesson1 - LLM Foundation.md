...
> **目标**：理解大模型"能力从何而来、成本因何而生"，为工程决策提供理性基线，明白"何时适用大模型"。
...
**工程视角**：这意味着你不需要为每个任务搭专用模型了。一个 API 接口 + 不同的 prompt 就能处理大部分语言任务。这极大降低了 AI 应用的工程门槛，也是为什么"提示词工程"（第二课）变得如此重要。
...
1. **钱**：API 按 token 计费。同样语义的中文通常比英文消耗多 1.5-2 倍 token，直接影响成本。

2. **容量**：128K 上下文窗口 ≠ 128K 字符。实际能放多少内容取决于分词效率。一个 128K 窗口大约能放 8-10 万中文字，但只是粗略估计。

3. **代码场景**：空格、缩进、长变量名都消耗 token。这就是为什么有些 AI 编码工具会对代码做预处理来压缩 token。

> **动手验证**：用 [OpenAI Tokenizer](https://platform.openai.com/tokenizer) 把你项目里的一段代码和一段中文需求文档分别粘贴进去，看看各消耗多少 token，建立直觉。
...
> **推荐选型流程**：Chatbot Arena 综合排名初筛 → 对应领域 Benchmark 专项筛选（代码看 SWE-Bench + Aider，推理看 MATH + GPQA）→ 在自己的 5-10 个真实 case 上实测做最终决策。

---

### 3.4 面向未来开发：哪些东西不会被模型吞掉
...
## 第五部分：实践 & 课后作业——在你的 Mac 上跑通第一个大模型
