...
先是好奇：
1. 模型不认识文字，它到底是怎么"读"代码的？
2. 它为什么能够“理解”代码中的空指针报错？为什么可以遵循我的指令和一步步思考？
3. output token 为什么比 input token 贵好几倍？追问为什么比首次提问快？如何优化token消耗和性能？

然后是工程决策：
4. 为什么不直接用最强的模型？
5. 选定模型后如何调参？
6. 前面讲了这么多，能不能自己跑一个模型试试？
...
先从第一个问题开始：模型不认识文字，它到底是怎么"读"代码的？跟着一条请求进模型走一遍，看看从你按下发送到第一个字出来，中间经过了哪些步骤。
...
>**动手验证**：用 [OpenAI Tokenizer](https://platform.openai.com/tokenizer) 或[HuggingFace - Tokenizer Playground](https://huggingface.co/spaces/Sagicc/tokenizer-playground)把代码放进去，看下去除注释、空格缩进等带来的影响。
...
Transformer 解决了这个问题：token 经过 Attention 的上下文交互之后，最终表示会随语境变化——同一个词在不同句子里的向量是不同的。下面看具体机制。
#### Q-K-V 机制
...
到这里，第一个问题有了答案：**模型怎么"读"代码的？**模型通过 **Tokenizer** 把代码切成 token，通过 **Embedding** 赋予语义，通过 **Attention** 让 user.name 回溯到 const user = res.ok ? ... : null 发现空值风险，再通过**多层 Transformer** 逐步深化推理——这就是它"读"代码的完整链路。但架构只解释了"怎么处理"，还没回答"为什么它懂空指针风险"——这是训练决定的。

---

## 第二部分：训练机制——能力从何而来

第一部分走完了模型从切词到推理的完整机制，但只解释了"怎么处理"——还没解释"为什么理解"。它为什么能够"理解"代码中的空指针报错？为什么可以遵循我的指令和一步步思考？**能力到底从何而来？**
...
**回答最初的问题**：**Pre-train 打基础，CPT 补知识，SFT 教行为，Alignment 调偏好。**

- 为什么能够“理解”代码中的空指针报错？答案在 Pre-train和CPT。

- 为什么可以准确返回JSON结果给我和一步步思考推理问题？答案在SFT。

- 为什么有时候review结论会很保守不指出严重错误？答案在Alignment。
...
前两部分回答了模型怎么"读"代码、能力边界在哪。第三个问题关于钱：output token 为什么比 input token 贵好几倍？追问为什么比首次提问快？如何优化 token 消耗和性能？
...
| **DualPath (DeepSeek)**         | 利用解码节点闲置带宽做 KV-Cache 双路加载，突破存储 I/O 瓶颈 | Agent 多轮长上下文场景吞吐提升近 2×       |
...
回答最初的问题：
- **output token 为什么比 input token 贵好几倍？**
	- output token 比 input 贵，是因为 Encode阶段可以并行但 Decode 阶段只能逐 token 串行生成；
- **追问为什么比首次提问快？**
	- 首次提问你等的那几秒是 Prefill 在并行处理整个输入；追问变快是因为 Prompt Cache 复用了上一轮的 KV Cache；
- **如何优化 token 消耗和性能？**
	- 把稳定内容（system prompt、规范、参考文档）放前面，把变化内容（用户输入、diff）放后面，最大化 Prompt Cache 命中；
	- 控制输出长度，因为 Decode 是主要成本来源；

---
## 第四部分：模型选型

前面三部分建立了理性基线——你知道了模型怎么处理代码、能力边界在哪、成本花在了哪里。现在回到第四个问题：**为什么不直接用最强的模型？** 你的 AI CR 工具每天跑上百个 PR，全用 Opus 一个月账单可能比招一个人还贵——这就是选型要解决的问题：**为什么要选型 → 需求是什么 → 怎么选**。
...
| 维度                        | 对应 Step | 关键问题                                          | 对选型的影响                                                                                                |
| ------------------------- | ------- | --------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| **输入模式**                  | 1       | 问答 / 代码 / 视觉 / 多模态？                           | 决定需要哪些输入模态                                                                                            |
| **上下文长度**                 | 1       | 单次输入有多长？需要处理整个代码库还是单个函数？                      | 长文档/大仓库需要 128k+ 窗口的模型；"能塞下"≠"都能用好"                                                                    |
| **提示词价格**                 | 1       | 每次调用的预算是多少？                                   | 直接划掉高于预算的模型层级                                                                                         |
| **延迟要求（Latency）**         | 1       | 用户能等几秒？P95 延迟要求？                              | Prefill阶段决定。旗舰模型 TTFT 通常比小模型慢                                                                         |
| **吞吐量要求**<br>（Throughput） | 1       | 每天要处理多少请求？高峰期并发量多大？                           | Decode 阶段决定。多用户共享一个API key ——并发越高，单用户分到的生成速度越低https://openrouter.ai/docs/api/reference/authentication |
| **部署模式**                  | 2       | API 调用还是本地？数据能出境吗？对幻觉的容忍度？                    | 本地部署只能选开源可自托管模型；医疗/法律/金融等高风险场景仍可用 LLM，但必须叠加验证层（如规则校验、人工复核）来兜底                                         |
| **任务场景**                  | 2       | 代码任务还是 Agent 任务？需要稳定 JSON / function calling？ | 代码任务优先 Coder 变体；Agent 场景优先 Reasoning 变体（长链推理 + 工具调用更稳）                                                |
| **显存 / 硬件**               | 2       | 本地有多少显存（或统一内存）？                               | 直接决定能跑的最大参数量和精度（本地部署专属，API 场景可跳过）                                                                     |
...
- **路由策略**：同一系统不必所有调用用同一模型。工程中常见三种路由实现（[参考](https://portkey.ai/blog/task-based-llm-routing/)）。路由的收益已有实证：Anthropic 在其[多模型路由指南](https://docs.anthropic.com/en/docs/build-with-claude/prompt-routing)中建议按任务复杂度分流到不同层级模型；实际工程中，团队通过规则路由将月度 LLM 开销降低 50%+ 而不影响输出质量的案例并不少见。
...
**快速选型 → 模型名称判断（本地部署） → 真实数据评测**

#### Step 1：用聚合平台快速选型

> **解决的约束**：输入模式、上下文长度、提示词价格、延迟要求、吞吐量要求——一次过滤完成初筛。

带着 4.2 定义的约束，用**模型聚合平台**做初筛是最高效的起点——这类平台汇集了数百个模型的定价、能力、上下文窗口等元数据，用约束条件做过滤，几分钟锁定候选池。这里以[OpenRouter](https://openrouter.ai/models) 为例。

OpenRouter是一个聚合了 200+ 模型的统一 API 平台，选型时有三个实用入口：

- **全局排行**（[/rankings](https://openrouter.ai/rankings), [/apps](https://openrouter.ai/apps) ）：反映真实工程场景下哪些模型被大量使用——比单纯看 Benchmark (如：[artificial analysis](https://artificialanalysis.ai/models))分数更贴近生产。
- **发现候选**（[/models](https://openrouter.ai/models)）：按任务类型、上下文长度、是否支持工具调用过滤，按流行度、价格、吞吐量、延迟等排序。几分钟看到满足约束的所有模型。
- **单模型详情**（[claude-opus-4.6](https://openrouter.ai/anthropic/claude-opus-4.6)）：每个模型有独立页面，列出该模型的实时性能、定价明细、上下文窗口、 Benchmark 成绩、支持的功能（工具调用、JSON 模式、视觉等），以及 Playground 可直接测试——是做候选对比时的标准查阅入口。
...
| **任务域变体**     | `Coder`、`Vision`、`Reasoning`        | 领域强化版本    | 代码任务优先 Coder；Agent 场景优先 Reasoning（长链推理 + 工具调用更稳） |
...
> **解决的约束**：质量——用你的真实业务数据做最终验证。

经过前两步，候选已经缩到 2–3 个。**不需要先看公开 Benchmark 再决定评测谁**——前沿模型就那几个，约束筛完已经够少了。公开 Benchmark 测的是通用能力分布，和你的任务分布大概率不一致；50 条真实样本的评测结果比任何榜单都更能预测模型在你任务上的表现（[如何构建评测集](https://hamel.dev/blog/posts/evals/)｜[LLM Evals FAQ](https://hamel.dev/blog/posts/evals-faq/)｜[AWS: Beyond Vibes](https://aws.amazon.com/blogs/machine-learning/beyond-vibes-how-to-properly-select-the-right-llm-for-the-right-task/)）。

**做法：**

1. **从生产数据中抽样 50–100 个典型 case**（AI CR 场景：真实 PR diff + 期望的 review 意见）
2. **定义评分维度**：准确性、格式合规、误报率——和业务指标对齐
3. **LLM-as-Judge 自动评测** + **人工抽检 10–20%**（[LLM-as-Judge 指南](https://www.anthropic.com/research/evaluating-ai-systems)｜[实践参考](https://hamel.dev/blog/posts/llm-judge/)）。注意 position bias：A/B 对比时随机化呈现顺序或交换顺序跑两次取平均
4. 分数差 1–2% 不值得换模型；**延迟和成本也要在真实数据上实测**

> [!info] 公开 Benchmark 仅作背景参考
> Benchmark 不适合作为选型步骤，但可以用来建立大致印象（[为什么不够用](https://www.evidentlyai.com/llm-guide/llm-benchmarks)｜[Benchmarks 2026](https://www.lxt.ai/blog/llm-benchmarks/)），了解大模型的能力发展方向。目前主流的benchmark包括以 **AIME / Codeforces** 为代表的慢思考与长逻辑推理；以 **SWE-bench**（真实工程开发）和 **OSWorld / WebArena**（系统与网页交互）为代表的 Agent 复杂操作闭环；以及以 **GPQA Diamond** 为代表的专家级前沿认知。注意，MMLU 和 HumanEval 等测验基础常识与简单生成的榜单均已高度饱和（90%+），不再是拉开头部梯队差距的指标。

**AI CR 场景**：收集 50 个历史 PR（覆盖不同复杂度和语言），用人工标注出"应该指出的问题"，作为评测集。让候选模型跑一遍，比较召回率和误报率——这比看 SWE-Bench 分数可靠得多。


**回到 AI CR 选型**：把三步走一遍
- 约束侧 CR 是异步秒级任务（延迟宽松）、单 PR 上下文几 k 到几十 k、每天几十到几百次调用、输出需要结构化；
- Step 1 在 OpenRouter 按"代码任务 + 32k+ 上下文 + 工具调用"过滤，公有仓库候选池一般是 Sonnet / GPT-4.1 / DeepSeek V3，私有仓库收敛到 Qwen2.5-Coder-32B / DeepSeek-Coder 等可自托管模型；
- Step 2 若走本地，32B + Q4 至少需要 ~24 GB 显存或统一内存；
- Step 3 用 50 个历史 PR 的人工标注集做召回率/误报率评测，再叠加 4.2 的规则路由（Haiku 扫风格、Sonnet 啃复杂逻辑），就是一个可落地的 CR 选型组合。

**回答最初的问题**：**为什么不直接用最强的模型？**——因为成本、延迟、合规、专项能力等约束往往比"质量最高"更优先。先用 4.2 的维度把约束写清楚，再用 4.3 的三步（聚合平台初筛 → 模型名字判可行性 → 真实数据评测）收敛候选，最后叠加路由策略让简单任务跑便宜模型。

---

## 第五部分：参数配置

模型选好了，第五个问题：**选定模型后如何调参？** 但在回答怎么调之前，先问一个更基础的问题——参数到底影响多大？
...
- Max Tokens 是成本与稳定性的“防火墙”： 通过动态限制输出长度，防止 AI 针对大批量代码产生冗余废话，从而降低 API 开销并避免因响应过长导致的流水线超时。

- Stop 参数是结构化解析的“断路器”： 在输出 JSON 或单行修复建议时，通过强制截断多余的客套话，确保结果能被 CI/CD 系统直接解析并自动标注到代码行。

- 两者共同实现了审查的“工程化鲁棒性”： 它们将 AI 从不可控的“聊天模式”切换为精准的“工具模式”，是实现自动化代码注入和大规模批量审查的技术基石。
...
### 5.5 常见任务配置速查
| **任务层级**         | **模型推荐**     | **Temperature** | **Max Tokens** | **Thinking (Reasoning)** | **备注**                        |
| ---------------- | ------------ | --------------- | -------------- | ------------------------ | ----------------------------- |
| **代码生成（函数级）**    | Sonnet       | 0.2             | 8192           | **开 (Budget 2048)**      | 增加边界条件思考，减少 Bug               |
| **代码审查 / Debug** | Sonnet       | 0.3             | **12288**      | 开 (Budget 8000)          | **核心：Max Tokens 必须覆盖 Budget** |
| **Agent 工具调用**   | Sonnet/Haiku | **0**           | 4096           | 关                        | 追求 100% 格式稳定性                 |
| **数据提取 / JSON**  | Haiku        | 0               | 4096           | 关                        | Haiku 响应极快，适合批量               |
| **头脑风暴 / 方案探索**  | Sonnet       | 0.9             | 4096           | 关                        | 需要发散思维，不建议开推理                 |
| **复杂架构 / 算法**    | Sonnet/Opus  | 0.4             | 16384          | **开 (Budget 12000+)**    | 深度思考复杂系统耦合                    |
...
**回答最初的问题**：**选定模型后如何调参？**——参数只占 4% 的影响（prompt 80%、上下文 15%），先改 prompt，再补上下文，最后才动参数。真要动时各有分工：
- **temperature**：控随机性，代码/JSON 用 0–0.2，对话/写作 0.6–0.8，头脑风暴 0.9+。
- **max_tokens + stop**：防截断兼成本闸门，按预期输出给 1.5× 余量，再用 `stop` 切掉废话。
- **thinking**：默认关，只在多步推理（跨文件 bug、复杂算法）才开。
- **logit_bias / prefilling**：日常不用，Agent 限制工具子集时用 prefilling，不破坏 KV Cache。
...
1. **模型选型**：根据任务和硬件选择本地模型。
2. **本地模型跑通**：用 Ollama 拉起一个模型并成功对话。
3. **AI 编码工具跑通**：用 OpenCode 完成一次代码相关对话。
...
1. **架构**：模型通过 Tokenizer 把代码切成 token，通过 Embedding 赋予基础语义，通过 Attention 关联上下文让 user.name 关联到 const user = res.ok ? ... : null 发现空值风险，再通过多层 Transformer 逐步深化推理——这就是它从"不认识文字"到"看出空指针"的完整链路。
2. **训练**：Pre-train 打基础，CPT 补知识，SFT 教行为，Alignment 调偏好。它之所以懂 null check，是因为预训练扫过了 GitHub 上数十亿行代码、CPT 阶段又用代码语料做了浸泡式深化；SFT 阶段教会模型逐步推理和指令遵循等能力；review 结论太保守可能是对齐税的原因。
3. **推理**：output token 比 input 贵，是因为 Prefill 阶段可以并行处理输入但 Decode 阶段只能逐 token 串行生成；首次请求等的那几秒是 Prefill 在并行处理整个输入，追问变快是因为 Prompt Cache 复用了上一轮的 KV Cache；为了优化 token 消耗和性能，应该把稳定内容（system prompt、规范、参考文档）放前面，把变化内容（用户输入、diff）放后面，最大化 Prompt Cache 命中；同时控制输出长度，因为 Decode 是主要成本来源。
4. **选型**：你没必要用最贵的旗舰模型——成本、延迟、合规、专项能力等约束往往比"质量最高"更优先。先确认需求（CR 是异步秒级任务（延迟宽松）、单 PR 上下文几 k 到几十 k、每天几十到几百次调用、输出需要结构化）；再利用聚合平台、模型名字进行选型，最后在真实业务上做 Evaluation。简单 CR 任务可以路由给便宜模型。
5. **参数**：参数只占 4% 的影响（prompt 80%、上下文 15%），先改 prompt 再补上下文，最后才动参数。真要动时：temperature 按任务定档（代码/CR 用 0.1–0.3，创意写作 0.6+），max_tokens + stop 防截断兼控成本，thinking 只在多步推理时才开，logit_bias / prefilling 用来在 Agent 场景限制工具子集且不破坏 KV Cache。
