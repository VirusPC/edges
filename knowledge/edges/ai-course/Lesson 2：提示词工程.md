...
```
本质认知 → 整体框架 → 提示词技术 → 系统提示词工程
  ↓            ↓            ↓               ↓
模型为什么    一次 API     11 种技术 +      分层架构与
不确定？     调用的结构    哪些已被模型吸收  真实工具案例
```
...
├─────────────────────────────────────────────────────┤
│  ② messages         对话历史                         │
│    ├─ user           用户的输入（任务、数据、上下文）    │
│    ├─ assistant      模型之前的回复                    │
│    └─ user           最新一轮用户输入                  │
├─────────────────────────────────────────────────────┤
│  ③ tools            可用工具的 JSON Schema 定义        │
│                     模型据此决定是否调用、如何调用       │
├─────────────────────────────────────────────────────┤
│  ④ parameters       控制参数                         │
│    ├─ model          选用哪个模型                     │
│    ├─ max_tokens     输出长度上限                     │
│    ├─ temperature    采样温度（创造性 vs 确定性）       │
│    └─ thinking / reasoning  推理增强配置（各厂商参数名不同）      │
└─────────────────────────────────────────────────────┘
```
...
```
┌───────────────────────────────────────────┐
│ system + tools（固定前缀）                   │ ← Prefill 一次
│                                            │   后续请求复用
│ 显式声明缓存（如 Claude 的 cache_control）   │   成本降低 ~90%
│ 或由平台自动缓存（如 OpenAI 的隐式缓存）      │
├───────────────────────────────────────────┤
│ messages（每次不同）                         │ ← 每次重新计算
└───────────────────────────────────────────┘
```
...
│  "你是一个资深代码审查工程师"           │
├─────────────────────────────────────┤
│  Layer 2：工具与资源（WITH WHAT）      │
│  可用工具列表、API 描述、知识库范围     │
├─────────────────────────────────────┤
│  Layer 3：行为约束（HOW）             │
│  NEVER 列表、ALWAYS 列表、边界条件     │
├─────────────────────────────────────┤
│  Layer 4：输出格式（WHAT FORMAT）     │
│  JSON Schema、Markdown 模板、字段定义  │
├─────────────────────────────────────┤
│  Layer 5：示例（SHOW ME）             │
│  2-3 个典型输入输出对                  │
└─────────────────────────────────────┘
