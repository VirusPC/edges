...
这一版的主要调整：
	•	开篇明确点出分类轴是作用域，让四层之间的递进关系（任务内 → 用户 → 项目 → 跨项目）自然成立，也呼应了 Team Memory 的命名逻辑。
	•	Working Memory 不再直接等同于上下文窗口，而是上下文中”被 Agent 主动读写的子集”，避开了和 system prompt、注入的检索片段混淆。
	•	在 User / Project 之间补了一段桥接说明，用”TypeScript 偏好”这个具体例子 + Claude Code 的文件分层实例，把”作用域分界”落到工程可感知的层面。​​​​​​​​​​​​​​​​
